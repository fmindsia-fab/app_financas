...
export function ReportsClient({ transactions, currentMes, currentAno }: Props) {
  ...
  const cardClass = "bg-white dark:bg-[#1e293b] rounded-2xl border border-slate-100 dark:border-slate-700/50 p-6 shadow-sm"
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`${cardClass} flex flex-wrap gap-3 items-center`}>
        ...
        <button
          ...
          className="ml-auto flex items-center gap-2 bg-[#0f172a] dark:bg-slate-700 hover:bg-slate-800 dark:hover:bg-slate-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition-colors"
        >
      ...
      <div className={cardClass}>
        <h2 className="font-bold text-slate-900 dark:text-white mb-6" style={{ fontFamily: 'Bricolage Grotesque, sans-serif' }}>
        ...
        <div className="h-64">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={barData} barCategoryGap="30%" barGap={4}>
              ...
              <XAxis dataKey="month" tick={{ fontSize: 12, fill: axisTickColor }} axisLine={false} tickLine={false} />
              <YAxis tick={{ fontSize: 12, fill: axisTickColor }} axisLine={false} tickLine={false} tickFormatter={v => `R$${(v / 1000).toFixed(0)}k`} />
              <Tooltip formatter={(v: number) => formatCurrency(v)} contentStyle={tooltipStyle} />
              <Legend formatter={v => <span style={{ fontSize: '12px', color: axisTickColor }}>{v === 'income' ? 'Receitas' : 'Despesas'}</span>} />
            ...
            <div className="h-56">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  ...
                  <Tooltip formatter={(v: number) => [formatCurrency(v), 'Total']} contentStyle={tooltipStyle} />
                </PieChart>
              </ResponsiveContainer>
            </div>
          )}
                ...
                <div key={row.name} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center justify-between mb-1">
                      <span className="text-sm font-medium text-slate-700 dark:text-slate-300 truncate">{row.name}</span>
                      <span className="text-sm font-semibold text-slate-900 dark:text-white ml-2">{formatCurrency(row.value)}</span>
                    </div>
